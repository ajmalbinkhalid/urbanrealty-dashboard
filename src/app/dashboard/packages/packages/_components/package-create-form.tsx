...
export function PackageCreateForm({ onSuccess }: AddSheetFormProps) {
  const packageTypes: EnumOption[] = Object.entries(PackageTypeEnum)
  ...
  const { form, mutation, CustomForm, resetForm } =
    useFormMutation<PackageFormData>({
      mutationFn: (data) => packageApi.createPackage(mapFormToApiPayload(data)),
      schema: PackageSchema,
      onSuccess,
      defaultValues: {
        type: String(PackageTypeEnum.Subscription),
        userType: Number(UserTypeEnum.Agent),
