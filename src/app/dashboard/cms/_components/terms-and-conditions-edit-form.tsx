...
export type TermsEditFormData = z.infer<typeof privacySchema>;

export function TermsEditForm({ data, onSuccess }: EditSheetFormProps<TCms>) {
  const queryClient = useQueryClient();
  const mutationFn = (formData: TermsEditFormData) => {
    return cmsApi.updateCms(
      buildCmsUpdatePayload("2", {
  ...
  const { form, CustomForm, mutation } = useFormMutation<TermsEditFormData>({
    mutationFn,
