...
export type PrivacyEditFormData = z.infer<typeof privacySchema>;

export function PrivacyPolicyEditForm({
  data,
  onSuccess,
}: EditSheetFormProps<TCms>) {
  const queryClient = useQueryClient();
  const mutationFn = (formData: PrivacyEditFormData) => {
    return cmsApi.updateCms(
      buildCmsUpdatePayload("1", {
  ...
  const { form, CustomForm, mutation } = useFormMutation<PrivacyEditFormData>({
    mutationFn,
    schema: privacySchema,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cms", "privacy-policy"] });
