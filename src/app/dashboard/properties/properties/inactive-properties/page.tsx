...
const Page = () => {
  ...
  const toggleStatusAction = DBRowActions.toggleStatus<TProperty>({
    toggleApi: propertyApi.toggleStatus,
    getChecked: (row) => row.status === 1,
  });

  const toggleFeaturedAction = DBRowActions.toggleFeatured<TProperty>({
    toggleApi: propertyApi.toggleFeatured,
    getChecked: (row) => row.isFeatured,
  });
  ...
  return (
    <DataTable
      ...
      fetchApiQueryParams={{ status: "inactive" }}
      pageSize={10}
      queryKey="properties-inactive"
